...
export default {
  ...
  methods: {
    getBook() {
      api.get(`/api/books/show?id=${this.$route.query.id}`,  {
        headers: {
          ...
          .then((result) => {
            ...
            this.form.isbn = result.data.isbn
    ...
    update() {
      api.put(`/api/books?id=${this.$data.form.id}`, this.$data.form, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('accessToken')}`
          }
      })
  ...
  <main class="grid place-items-center">
    ...
    <div class="md-5 ml-5 mr-5 mt-5 bg-gray-900 rounded w-2/3" v-if="success">
      <div class="pd-5 pl-5 pr-5 pt-2 text-gray-100 mb-2">
        <h5>The book is edited successfully!</h5>
      </div>
    </div>
  </main>
</template>